...
func testBIRSerializationRoundtrip(t *testing.T, testPair test_util.TestCase) {
	...
	if birPkg == nil || compileErr != nil {
		// Compilation failed — compare compile-time output against expected.
		...
		result := evaluateTestResult(expectedStdout, expectedStderr, stdoutBuf.String(), stderrBuf.String())
		if !result.success {
			t.Errorf("compile-phase output mismatch for %s", testPair.InputPath)
		}
		return
	}
