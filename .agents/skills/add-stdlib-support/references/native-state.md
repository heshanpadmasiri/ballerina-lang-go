...
**Reference implementation:** `lib/stdlibs/ballerina/crypto/0.0.1/go1.27/native/keydata.go` (`setKeyData`/`keyDataOf`), which recovers the parsed `*rsa.PrivateKey`/`*ecdsa.PrivateKey`/etc. behind a `crypto:PrivateKey`/`PublicKey` record.
