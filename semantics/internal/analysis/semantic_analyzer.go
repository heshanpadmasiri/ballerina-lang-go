...
func hasSingleApplicableAttachPointListType(cx semtypes.Context, svc *ast.BLangService, attachPointTy semtypes.SemType) bool {
	...
	for i, segment := range svc.AbsoluteResourcePath {
		members[i] = semtypes.ListMemberInfo{Index: i, ValueType: semtypes.StringConst(segment.Value)}
	}
	found := false
...
func isImmutableField(tyCtx semtypes.Context, field *ast.BLangVariable) bool {
	return field.IsFinal() && semtypes.IsSubtype(tyCtx, field.GetDeterminedType(), semtypes.ValReadonly)
}
